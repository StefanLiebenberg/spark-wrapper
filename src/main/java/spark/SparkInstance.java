 ...
 *
 * 1. Made class public.
 * 2. Made init function not use singleton instance of routeMatcher.
 */
...
public final class SparkInstance extends Routable {
    ...
    public synchronized void init() {
        if (!initialized) {
            ...
            if (!ServletFlag.isRunningFromServlet()) {
                new Thread(() -> {
                    MatcherFilter matcherFilter = new MatcherFilter(routeMatcher, false, hasMultipleHandlers());
                    matcherFilter.init(null);
                    JettyHandler handler = new JettyHandler(matcherFilter);
                    server = new JettySparkServer(handler);
                    server.ignite(ipAddress, port, sslStores, staticFileFolder, externalStaticFileFolder, latch,
                                  maxThreads, minThreads, threadIdleTimeoutMillis, webSocketHandlers,
